...
var mainApp = Backbone.View.extend({
	...
	_item:Backbone.View.extend({// View  отвечающая за элементы списка
		...
		initialize:function (options) {
			...
			this.model.on('change:date', this.reRender, this);
			this.model.on('change:time', this.reRender, this);
...
var Store = function (name) {
	...
	_.isEmpty(this.data)?this.import(document.rasp_2012):'';
};
...
var ItemList = Backbone.Collection.extend({
	...
	comparator:function (model) {//Определение сортировки колллекции по дате
		...
		var d = new Date(date[2], date[1]-1, date[0], time[0], time[1], 0, 0);
		return d.getTime();
	},
